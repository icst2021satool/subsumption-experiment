...
* **checkoutbatch.sh**. This script checks out all the versions 1b (ou 25b, in the case of Collections) from Defect4J's repository. Note that Weka is not checked out since its source and object code are already in this repository.

* **expandbatch.sh**. The coverage data generated by Jaguar occupies too much space. Thus, they are zipped in this repository. This script expands Jaguar's coverage files.
...
* **assertbatch.sh**. Calculates DUA coverage for all Defects4J's 1b (or 25b, in the case of Collections) versions, excepting Lang and Mockito. It also calculates DUA coverage for Weka 3.8. This script calls **assert_project.sh**, which is a script that provides the correct parameters for each project to python program **assert_jaguar.py**.
